...
2. Run the validator and resolve any errors:
   ...
   python3 tools/validate.py iso3166.json --strict-count
    ...
    Cite a first-party source in the commit message:
    ISO 3166 online browsing platform, the UN M49 overview export,
    IANA's ccTLD registry, or the ITU's E.164 list.
...
When ISO assigns a new alpha-2 code:
    ...
    Add the entry to the active array in iso3166.json.

    Update tools/parse_source.py:
    bash
    ...
    Bump VERSION and meta.version (minor bump).

    Add a CHANGELOG.md entry citing the ISO amendment.
...
Every push to main runs 13 jobs:
...
any release:
    ...
    The full CI run for the exact commit being tagged must be green.

    python3 tools/check_version_consistency.py must exit 0.

    python3 tools/validate.py iso3166.json --strict-count must exit 0.

    bash tools/check_cross_language.sh US must exit 0.
